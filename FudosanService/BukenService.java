...
//物件一覧を取得
    ...
    public List<BukenBean> listInfo() {
        ...
        try {
            jc.getDbcom();
            String sql = "SELECT * FROM Property where status is null order by property_id";
...
//管理者物件検索一覧
    ...
    //ユーザログインチェック
//    public List<UsersBean> yuzatiku(String id, String password) {
//        JdbcConn jc = new JdbcConn();
...
//売上一覧
    ...
    public List<TintaiTyukaiBean> listInfoSaleContract() {
        ...
        try {
            ...
            String sql = "select S.contract_id, P.status, S.property_id, S.buyer_name, S.contract_date, S.price, C.commission_id, C.commission_fee from \n"
                    + "SaleContract S join Commission C on S.contract_id = C.contract_id join Property P on S.property_id = P.property_id\n"
                    + "where P.status is not null order by S.contract_id asc ";
...
//            sb.append("homeid =  " + bk.getHomeid() + ",");
    ...
    //賃貸ステータス更新
    public void tintaisutetasuUpdate(TintaiTyukaiBean bb) throws SQLException {
        JdbcConn jc = new JdbcConn();
        ...
        try {
            ...
            sb.append(" where ");
            sb.append(" property_id = " + bb.getContractId());
            sb.append(";");
            System.out.println(sb.toString());
            jc.cud(sb.toString());
...
//                    + "values('" + id + "','" + password + "','" + phone + "')";
            ...
            String sql = "SELECT * FROM Property where price < " + id + "";
            try {
                ResultSet resultSet = jc.tt(sql);
                while (resultSet.next()) {
                    BukenBean bukenBeanlist = new BukenBean();
    ...
    public List<BukenBean> areakensaku(String id) {
        ...
        try {
            ...
            String sql = "SELECT * FROM Property where property_area < " + id + "";
            try {
                ResultSet resultSet = jc.tt(sql);
                while (resultSet.next()) {
                    BukenBean bukenBeanlist = new BukenBean();
